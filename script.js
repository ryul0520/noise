...
window.onload = function() {
    ...
    const keys = {};
    const resetButton = { x: 0, y: 0, width: 80, height: 40 }; // ✨ 리셋 버튼 객체
    ...
    function renderWorld() { ctx.fillStyle = '#000'; ctx.fillRect(0, 0, width, height); const physicalObjects = worldObjects.filter(o => o.isPhysical); physicalObjects.forEach(obj => { const screenX = Math.floor(obj.worldX - camera.x); const screenY = Math.floor(obj.worldY - camera.y); if (screenX + obj.width < 0 || screenX > width || screenY + obj.height < 0 || screenY > height) return; ctx.fillStyle = '#222'; ctx.fillRect(screenX, screenY, obj.width, obj.height); }); }
    ...
    function drawResetButton() {
        ...
        ctx.fillText('RESET', resetButton.x + resetButton.width / 2, resetButton.y + resetButton.height / 2);
    }
    ...
    function animate(time) {
        ...
        drawCoins(time); 
        drawPlayer(time);
        drawResetButton(); // ✨ 리셋 버튼 그리기
        if (!gameCleared) { drawControlButtons(); drawStageUI(); }
    ...
    function saveGameState() {
        const gameState = {
            stage: currentStage,
            record: sessionRecordX
        };
        localStorage.setItem('noiseGameState', JSON.stringify(gameState));
    }
    function loadGameState() {
        const savedState = localStorage.getItem('noiseGameState');
        if (savedState) {
            const gameState = JSON.parse(savedState);
            currentStage = gameState.stage || 1;
            sessionRecordX = gameState.record || 0;
        }
    }
