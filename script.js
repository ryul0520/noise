window.onload = function() {
    ...
    // ✨ (UI 위치 변경) 스테이지 UI를 화면 상단 중앙으로 이동
    function drawStageUI() {
        const uiWidth = 140;
        const uiHeight = 40;
        const x = width / 2 - uiWidth / 2;
        const y = 20;

        ctx.fillStyle = 'rgba(0, 0, 0, 0.4)'; 
        ctx.strokeStyle = 'rgba(255, 255, 255, 0.5)'; 
        ctx.lineWidth = 2;
        ctx.fillRect(x, y, uiWidth, uiHeight);
        ctx.strokeRect(x, y, uiWidth, uiHeight);
        ctx.font = 'bold 22px sans-serif'; 
        ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
        ctx.textAlign = 'center'; 
        ctx.textBaseline = 'middle';
        ctx.fillText('STAGE ' + currentStage, width / 2, y + uiHeight / 2);
    }
